...
import { getCompositeKey } from "../table-keys.js";
...
async function loadDirectRelationValues(database, sourceRows, relation) {
    ...
    let relatedRows = await loadLinkedRows(database, createQuery(relation.targetTable), relation, relation.targetKey, sourceTuples);
...
async function loadHasManyThroughValues(database, sourceRows, relation) {
    ...
    let throughRows = await loadLinkedRows(database, createQuery(throughRelation.targetTable), throughRelation, throughRelation.targetKey, sourceTuples);
    ...
    let relatedRows = await loadLinkedRows(database, createQuery(relation.targetTable), relation, relation.through.throughTargetKey, throughTuples);
...
async function loadLinkedRows(database, query, relation, targetColumns, tuples) {
    let next = query;
    let predicate = buildLinkPredicate(targetColumns, tuples);
    if (predicate) {
        next = next.where(predicate);
    }
    return loadRowsWithRelationsForQuery(database, applyRelationModifiers(next, relation, {
