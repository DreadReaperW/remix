...
import { asQueryTableInput, getPrimaryKeyWhere, getPrimaryKeyWhereFromRow, normalizeOrderByInput, resolveCreateRowWhere, toWriteResult, } from "./helpers.js";
import { createInitialQueryState, QueryBuilder } from "./query-builder.js";
const databaseInternalOptionsKey = Symbol('databaseInternalOptions');
export function withDatabaseInternals(options, internal) {
    let databaseOptions = {
        now: options?.now,
    };
...
function hasDatabaseInternals(options) {
...
export class Database {
    ...
    constructor(adapter, options) {
        let internal = options && hasDatabaseInternals(options) ? options[databaseInternalOptionsKey] : undefined;
    ...
    async transaction(callback, options) {
        if (!this.#token) {
            ...
            let tx = new Database(this.#adapter, withDatabaseInternals({ now: this.#now }, {
                token,
