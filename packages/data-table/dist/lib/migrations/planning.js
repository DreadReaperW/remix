import { normalizeChecksum } from "./journal-store.js";
export function listMigrations(migrations) {
    return Array.isArray(migrations)
        ? [...migrations].sort((left, right) => left.id.localeCompare(right.id))
        : migrations.list();
}
export function assertStepOption(step) {
    if (step === undefined) {
        return;
    }
...
export function assertMigrateOptions(options) {
...
export function assertTargetOption(migrations, to) {
    if (!to) {
        return;
    }
...
export function assertNoMigrationDrift(migrations, journal) {
...
export function selectMigrationsToRun(direction, migrations, journal, options) {
...
export function createMigrationStatusEntries(migrations, journal) {
