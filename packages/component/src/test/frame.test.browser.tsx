...
describe('run', () => {
  ...
  it('hydrates a single component', async (t) => {
    ...
    let loadModule = t.mock(() => Promise.resolve(Counter))

    let frame = run(document, { loadModule })
    await frame.ready()

    assert.deepEqual(loadModule.mock.calls[0]?.arguments, ['/js/counter.js', 'Counter'])
    ...
    assert.equal(button?.textContent, 'Count: 6')

    frame.dispose()
  })

  it('hydrates multiple components', async (t) => {
    ...
    let loadModule = t.mock(() => Promise.resolve(Button))
    ...
    assert.equal(buttons[0]?.textContent, 'First clicked!')
    assert.equal(buttons[1]?.textContent, 'Second')

    frame.dispose()
  })

  it('hydrates ready modules before slower modules while ready() stays pending', async (t) => {
    ...
    let loadModule = t.mock((moduleUrl: string, exportName: string) => {
    ...
    slowButton.click()
    app.flush()
    assert.equal(slowButton.textContent, 'Slow')
  ...
  it('handles complex props', async (t) => {
    ...
    let loadModule = t.mock(() => Promise.resolve(Card))

    let frame = run(document, { loadModule })
    await frame.ready()

    assert.deepEqual(loadModule.mock.calls[0]?.arguments, ['/js/card.js', 'Card'])
    assert.equal(document.querySelector('h2')?.textContent, 'Test')
    assert.equal(document.querySelector('p')?.textContent, 'Count: 42')
    assert.equal(document.querySelectorAll('li').length, 3)

    frame.dispose()
  })

  it('ready() does not wait for hydration markers from later frame templates', async (t) => {
    ...
    lateButton.click()
    app.flush()
    assert.equal(lateButton.textContent, 'Late')
  ...
  it('does nothing when no rmx-data script exists', async (t) => {
    document.body.innerHTML = '<div>No hydration here</div>'

    let loadModule = t.mock()

    let frame = run(document, { loadModule })
    await frame.ready()

    assert.equal(loadModule.mock.calls.length, 0)

    frame.dispose()
  })

  it('does nothing when rmx-data has no hydration data', async (t) => {
    document.body.innerHTML = `
  ...
  it('adopts existing DOM nodes during hydration', async (t) => {
    ...
    assert.equal(spanAfterHydration, existingSpan)

    frame.dispose()
  })

  it('replaces pending frame regions when streamed templates arrive', async (t) => {
    let stream = renderToStream(
      <div>
        <h1>Title</h1>
    ...
    invariant(h1 && p && nav)
    assert.equal(nav.textContent, 'Loading...')

    let frame = run(document, { loadModule: t.mock() })
    await frame.ready()
    ...
    assert.equal(nav.textContent, 'Loaded')

    frame.dispose()
  })

  it('merges hydration data across multiple rmx-data scripts', async (t) => {
  ...
  it('ignores prototype-polluting keys when merging rmx-data scripts', async (t) => {
    function A() {
    ...
    assert.equal(loadModule.mock.calls.length, 1)
    assert.deepEqual(loadModule.mock.calls[0]?.arguments, ['/a.js', 'A'])
  ...
  it('reloads a frame region and preserves static DOM nodes', async () => {
    ...
    assert.ok(document.querySelector(`template#${frameId}`))
    ...
    assert.equal(document.querySelector('p')?.textContent, 'Server: 1')
    invariant(reload)
    ...
    assert.equal(document.querySelector('p')?.textContent, 'Server: 2')

    // Static elements are the exact same DOM nodes — not replaced.
    assert.equal(document.querySelector('section'), section)
    assert.equal(document.querySelector('h2'), heading)
    assert.equal(document.querySelector('p'), paragraph)
    assert.equal(document.querySelector('ul'), list)
    assert.equal(document.querySelectorAll('li')[0], items[0])
    assert.equal(document.querySelectorAll('li')[1], items[1])
    assert.equal(document.querySelector('button'), button)

    // Static text preserved.
    assert.equal(heading.textContent, 'Activity')
  ...
  it('clears frame content when reload resolves to an empty stream', async () => {
    ...
    await clientFrame.ready()
    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.equal(document.getElementById('frame-value')?.textContent, 'Initial content')

    invariant(reload)
    await reload()
    await new Promise((resolve) => setTimeout(resolve, 0))

    assert.equal(document.getElementById('frame-content'), null)
    assert.equal(document.getElementById('frame-value'), null)
  ...
  it('looks up named adjacent frames from handle.frames.get(name)', async () => {
    ...
    let RowAction = clientEntry(
      ...
      function RowAction(handle: Handle) {
        reloadSummary = async () => {
          assert.equal(handle.frames.get('missing-frame'), undefined)
    ...
    assert.equal(document.querySelector('#summary')?.textContent, 'Summary: 1')

    invariant(reloadSummary)
    await reloadSummary()
    await new Promise((resolve) => setTimeout(resolve, 0))

    assert.equal(document.querySelector('#summary')?.textContent, 'Summary: 2')
  ...
  it('exposes the root frame as handle.frames.top', async () => {
    ...
    let ReloadTop = clientEntry(
      ...
      function ReloadTop(handle: Handle) {
        assertTopFrame = () => {
          assert.notEqual(handle.frames.top, handle.frame)
        }
  ...
  it('dispatches reloadStart and reloadComplete events for handle.frame and handle.frames.get(name)', async () => {
    ...
    assert.equal(document.getElementById('summary-events')?.textContent, 'Summary: 1')

    invariant(triggerReloads)
    await triggerReloads()
    await new Promise((resolve) => setTimeout(resolve, 0))

    assert.equal(summaryReloadStartEvents, 1)
    assert.equal(rowReloadStartEvents, 1)
    assert.equal(summaryReloadCompleteEvents, 1)
    assert.equal(rowReloadCompleteEvents, 1)
    assert.equal(document.getElementById('summary-events')?.textContent, 'Summary: 2')
  ...
  it('aborts stale frame reloads when reload is re-entered', async () => {
    ...
    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.equal(firstReturnedSignal, firstSignal)
    assert.equal(firstReturnedSignal.aborted, true)

    // First reload should be ignored because it was superseded.
    assert.equal(document.getElementById('reload-value')?.textContent, 'Initial')

    resolveSecondReloadContent('<section><p id="reload-value">Fresh</p></section>')
    let secondReturnedSignal = await secondReloadPromise
    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.equal(secondReturnedSignal, secondSignal)
    assert.equal(secondReturnedSignal.aborted, false)

    assert.equal(document.getElementById('reload-value')?.textContent, 'Fresh')
    clientFrame.dispose()
  })
  ...
  it('hoists head-managed elements when a frame reloads', async () => {
    ...
    assert.equal(document.head.querySelector('title')?.textContent, 'Frame title 1')
    assert.equal(
    ...
    assert.equal(document.head.querySelector('script[type="text/javascript"]'), null)
    assert.ok(document.querySelector('main script[type="text/javascript"]'))
    assert.equal(document.querySelector('main title'), null)
    assert.equal(document.querySelector('main meta[name="frame-description"]'), null)
    assert.equal(document.querySelector('main script[type="application/ld+json"]'), null)
    ...
    assert.equal(titles[0]?.textContent, 'Frame title 1')
    assert.equal(titles[1]?.textContent, 'Frame title 2')

    let metas = document.head.querySelectorAll('meta[name="frame-description"]')
    assert.equal(metas.length, 2)
    assert.equal(metas[0]?.getAttribute('content'), 'frame-1')
    assert.equal(metas[1]?.getAttribute('content'), 'frame-2')

    let ldJsonScripts = document.head.querySelectorAll('script[type="application/ld+json"]')
    assert.equal(ldJsonScripts.length, 2)
    assert.equal(ldJsonScripts[0]?.textContent, '{"count":1}')
    assert.equal(ldJsonScripts[1]?.textContent, '{"count":2}')
    assert.equal(document.querySelector('p')?.textContent, 'Frame body 2')

    clientFrame.dispose()
  })

  it('hydrates client entries in blocking frame content without redefining virtual roots', async (t) => {
    let Counter = clientEntry('/js/counter.js#Counter', function Counter() {
      return () => (
    ...
    let loadModule = t.mock(async () => modulePromise)
    let clientFrame = run(document, { loadModule })

    await new Promise((resolve) => setTimeout(resolve, 0))
    resolveModule(Counter)

    assert.equal(await clientFrame.ready(), undefined)

    let button = document.getElementById('counter') as HTMLButtonElement | null
    invariant(button)
    assert.ok(button.textContent?.includes('Count'))
    assert.ok(loadModule.mock.calls.length > 0)

    clientFrame.dispose()
  })

  it('hydrates components without waiting for pending frames', async (t) => {
    let Counter = clientEntry(
      '/js/counter.js#Counter',
      function Counter(handle: Handle, setup: number) {
        let count = setup
    ...
    assert.equal(document.getElementById('frame')!.textContent, 'Loading…')

    let clientFrame = run(document, {
      loadModule: t.mock(() => Promise.resolve(Counter)),
    })
    await clientFrame.ready()
    ...
    assert.equal(document.getElementById('frame')!.textContent, 'Loaded!')
  ...
  it('pending frames resolve independently as their templates arrive', async (t) => {
    ...
    assert.equal(document.getElementById('fast')!.textContent, 'Loading fast…')
    assert.equal(document.getElementById('slow')!.textContent, 'Loading slow…')

    let clientFrame = run(document, { loadModule: t.mock() })
    await clientFrame.ready()
    ...
    assert.equal(document.getElementById('fast')!.textContent, 'Fast loaded')
    assert.equal(document.getElementById('slow')!.textContent, 'Loading slow…')
    ...
    assert.equal(document.getElementById('fast')!.textContent, 'Fast loaded')
    assert.equal(document.getElementById('slow')!.textContent, 'Slow loaded')

    clientFrame.dispose()
  })

  it('pending frames resolve while modules are still loading', async (t) => {
    ...
    assert.ok(loadModuleFn.mock.calls.length > 0)
    ...
    assert.equal(moduleLoaded, true)

    clientFrame.dispose()
  })

  it('hydrates a component inside a nested frame', async (t) => {
    let Counter = clientEntry(
      '/js/counter.js#Counter',
      function Counter(handle: Handle, setup: number) {
        let count = setup
    ...
    assert.equal(document.getElementById('outer')!.textContent, 'Loading outer…')
    ...
    assert.equal(document.getElementById('inner')!.textContent, 'Loading inner…')
    ...
    button.click()
    clientFrame.flush()
    assert.equal(button.textContent, 'Count: 11')

    clientFrame.dispose()
  })

  it('deeply nested frames resolve independently at each level', async (t) => {
    ...
    assert.equal(document.getElementById('title')!.textContent, 'Page')
    assert.equal(document.getElementById('outer')!.textContent, 'Loading outer…')
    ...
    assert.equal(document.getElementById('outer-content')!.textContent, 'Outer loaded')
    assert.equal(document.getElementById('middle')!.textContent, 'Loading middle…')
    ...
    assert.equal(document.getElementById('middle-content')!.textContent, 'Middle loaded')
    assert.equal(document.getElementById('inner')!.textContent, 'Loading inner…')
    ...
    assert.equal(document.getElementById('outer-content')!.textContent, 'Outer loaded')
    assert.equal(document.getElementById('middle-content')!.textContent, 'Middle loaded')
    assert.equal(document.getElementById('inner-content')!.textContent, 'Inner loaded')
  ...
  it('reloads a frame that is nested inside another frame', async (t) => {
    ...
    let clientFrame = run(document, {
      loadModule: t.mock(() => Promise.resolve(ReloadButton)),
      resolveFrame: renderInner,
    })
    ...
    assert.equal(document.getElementById('outer-text')!.textContent, 'Outer')
    assert.equal(document.getElementById('inner-fallback')!.textContent, 'Loading…')
    ...
    assert.equal(document.getElementById('inner-text')!.textContent, 'Render 1')
    ...
    assert.equal(document.getElementById('inner-text')!.textContent, 'Render 2')
    assert.equal(document.getElementById('outer-text')!.textContent, 'Outer')

    clientFrame.dispose()
  })

  it('renders a client-created Frame with createRoot frameInit', async (t) => {
    let { container } = t.render(
      <Frame src="/client-frame" fallback={<p id="fallback-frame">Loading…</p>} />,
      {
        frameInit: {
    ...
    assert.equal(container.querySelector('#fallback-frame')?.textContent, 'Loading…')

    await new Promise((resolve) => setTimeout(resolve, 0))

    assert.equal(container.querySelector('#resolved-frame')?.textContent, 'Resolved frame')
  })
  ...
  it('renders a client-created Frame with createRangeRoot frameInit', async () => {
    ...
    assert.equal(host.querySelector('#fallback-range-frame')?.textContent, 'Loading…')
    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.equal(host.querySelector('#resolved-range-frame')?.textContent, 'Resolved range frame')
    root.dispose()
  })
  ...
  it('dispatches a clear error for createRoot Frame without frameInit', () => {
    ...
    assert.ok((error as Error).message.includes('Cannot render <Frame /> without frame runtime'))
  })
  ...
  it('throws from the root runtime resolveFrame fallback without frameInit', (t) => {
    ...
    t.render(<CaptureRuntime />)

    assert.notEqual(runtime, undefined)
    assert.throws(
      () => runtime!.resolveFrame('/missing-runtime'),
      /Cannot render <Frame \/> without frame runtime/,
    )
  })

  it('logs a clear error when hydrating client entries without loadModule', async (t) => {
    let Counter = clientEntry(
      '/js/counter.js#Counter',
      function Counter(handle: Handle, setup: number) {
        let count = setup
    ...
    let consoleError = t.spyOn(console, 'error', () => {})

    t.render(<Frame src="/counter-frame" fallback={<p>Loading…</p>} />, {
      frameInit: {
  ...
  it('logs a clear error when loadModule resolves to a non-function export', async (t) => {
    let Counter = clientEntry(
      '/js/counter.js#Counter',
      function Counter(handle: Handle, setup: number) {
        let count = setup
    ...
    t.render(<Frame src="/bad-export-frame" fallback={<p>Loading…</p>} />, {
      frameInit: {
        resolveFrame: async () => html,
  ...
  it('reloads client-created Frame in place when src changes', async (t) => {
    let [nextFramePromise, resolveNextFrame] = withResolvers<string>()

    let { container, root } = t.render(
      <Frame src="/a" fallback={<p id="fallback-a">Loading A…</p>} />,
      {
        frameInit: {
    ...
    assert.equal(container.querySelector('#fallback-a')?.textContent, 'Loading A…')

    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.equal(container.querySelector('#frame-a')?.textContent, 'A')

    let frameA = container.querySelector('#frame-a')
    ...
    assert.equal(container.querySelector('#fallback-b'), null)
    assert.equal(container.querySelector('#frame-a'), frameA)

    resolveNextFrame('<p id="frame-b">B</p>')

    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.equal(container.querySelector('#frame-b')?.textContent, 'B')
    root.dispose()
  })
  ...
  it('renders a client-created Frame after run() from a hydrated entry component', async () => {
    ...
    assert.equal(document.getElementById('post-run-fallback')?.textContent, 'Loading post-run…')

    await new Promise((resolve) => setTimeout(resolve, 0))

    assert.equal(document.getElementById('post-run-loaded')?.textContent, 'Post-run loaded')
    app.dispose()
  })

  it('does not duplicate initially-mounted Frame hydration in a client entry', async (t) => {
    ...
    assert.equal(document.querySelectorAll('#outer-fallback').length, 1)

    let clientResolveFrame = t.mock(
    ...
    await app.ready()
    assert.equal(clientResolveFrame.mock.calls.length, 0)
    ...
    assert.equal(clientResolveFrame.mock.calls.length, 0)
    assert.equal(document.querySelectorAll('#outer-root').length, 1)
    assert.equal(document.querySelectorAll('#nested-fallback').length, 1)
    ...
    assert.equal(clientResolveFrame.mock.calls.length, 0)
    assert.equal(document.querySelectorAll('#nested-loaded').length, 1)
    assert.equal(document.querySelectorAll('#nested-fallback').length, 0)
  ...
  it('renders Frame semantics from entry children during initial hydration', async () => {
    ...
    assert.equal(document.getElementById('child-frame')?.textContent, 'Loading child frame…')
    ...
    assert.equal(document.getElementById('child-frame')?.textContent, 'Loaded child frame')
    app.dispose()
  })

  it('does not dispose managed stylesheets when removing a client-created Frame', async (t) => {
    ...
    let { container, root } = t.render(<Shell />, {
      frameInit: {
    ...
    let button = container.querySelector('#toggle-frame')
  ...
  it('streams client resolveFrame templates and updates nested placeholders incrementally', async () => {
    ...
    assert.equal(document.getElementById('outer')?.textContent, 'Reloaded outer')
    assert.equal(document.getElementById('nested')?.textContent, 'Loading nested…')

    releaseSecondChunk(secondChunk.value)
    await reloadPromise
    await new Promise((resolve) => setTimeout(resolve, 0))

    assert.equal(document.getElementById('nested')?.textContent, 'Nested loaded')
  })

  it('cancels stale client frame streams when src changes', async (t) => {
    let rootContainer = document.createElement('div')
    document.body.appendChild(rootContainer)
    ...
    assert.equal(container.querySelector('#fallback')?.textContent, 'Loading…')

    let switchButton = container.querySelector('#switch-src')
    invariant(switchButton instanceof HTMLButtonElement)
    switchButton.click()
    root.flush()
    await new Promise((resolve) => setTimeout(resolve, 0))

    assert.equal(container.querySelector('#result')?.textContent, 'Fast result')
