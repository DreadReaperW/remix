...
export function compilePostgresMigrationOperations(operation) {
...
function compileCreateTableStatements(operation) {
...
function compileAlterTableStatements(operation) {
    let statements = [];
...
function compileAlterTableChange(table, change) {
    ...
    switch (change.kind) {
        ...
        case 'renameColumn':
            return {
                text: prefix + 'rename column ' + quoteIdentifier(change.from) + ' to ' + quoteIdentifier(change.to),
                values: [],
            };
        case 'dropColumn':
            return {
        ...
        case 'setTableComment':
            return {
                text: 'comment on table ' + quoteTableRef(table) + ' is ' + quoteLiteral(change.comment),
                values: [],
            };
    }
}
function compileTableConstraints(operation) {
    let constraints = [];
...
function compileCheckConstraint(name, expression) {
    return 'constraint ' + quoteIdentifier(name) + ' check (' + expression + ')';
}
function compileForeignKeyConstraint(constraint) {
    return ('constraint ' +
