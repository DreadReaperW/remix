...
export async function transformModule(record, args) {
    ...
    try {
        ...
        if (mayContainCommonJSModuleGlobals(sourceText) && isCommonJS(analysis.rawCode)) {
            throw createScriptServerCompilationError(`CommonJS module detected: ${resolvedPath}. ` +
        ...
        if (!stableUrlPathname) {
            throw createScriptServerCompilationError(`Module ${record.identityPath} is outside all configured routes.`, {
...
async function analyzeModuleSource(sourceText, resolvedPath, transformOptions, options) {
    ...
    try {
        ...
        if (isScriptServerCompilationError(error))
            throw error;
        throw createScriptServerCompilationError(`Failed to transform module ${resolvedPath}. ${formatUnknownError(error)}`, {
            cause: error,
...
async function minifyModule(rawCode, resolvedPath, target, sourceMaps) {
    try {
        ...
        if (isScriptServerCompilationError(error))
            throw error;
        throw createScriptServerCompilationError(`Failed to minify module ${resolvedPath}. ${formatUnknownError(error)}`, {
            cause: error,
            code: 'MODULE_TRANSFORM_FAILED',
        });
    }
...
function getJsxOptions(resolvedPath, compilerOptions) {
    ...
    if (jsx === 'preserve' || jsx === 'react-native') {
        throw createScriptServerCompilationError(`Unsupported tsconfig compilerOptions.jsx = "${jsx}" for ${resolvedPath}. ` +
...
function assertNoCompilerErrors(errors, resolvedPath, operation) {
    if (!errors || errors.length === 0)
        return;
    throw createScriptServerCompilationError(`Failed to ${operation} module ${resolvedPath}. ${errors[0].message ?? 'Unknown error'}`, {
        code: 'MODULE_TRANSFORM_FAILED',
    });
}
...
function toTransformFailedError(error, resolvedPath) {
    if (isScriptServerCompilationError(error))
        return error;
    return createScriptServerCompilationError(`Failed to transform module ${resolvedPath}. ${formatUnknownError(error)}`, {
        cause: error,
        code: 'MODULE_TRANSFORM_FAILED',
    });
}
